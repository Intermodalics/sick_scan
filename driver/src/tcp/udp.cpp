...
//
bool Udp::write(UINT8* buffer, UINT32 numberOfBytes)
{
	INT32 bytesSent;
	bool result;
#ifdef _MSC_VER
	...
	{
		printWarning("Udp::write: Failed to send data to socket.");
		result = false;
	}
	else
	{
		// Erfolg
		printInfoMessage("Udp::write: Sent " + toString(numberOfBytes) + " bytes to client.", m_beVerbose);
		result = true;
	}
...
//
void Udp::setDisconnectCallbackFunction(DisconnectFunction discFunction, void* obj)
{
...
bool Udp::isOpen()
{
	...
	{
//		printInfoMessage("Udp::isOpen: Reporting open connection.", m_beVerbose);
		return true;
	}

//	printInfoMessage("Udp::isOpen: Reporting no connection.", m_beVerbose);
	return false;
}
...
//
void Udp::setReadCallbackFunction(Udp::ReadFunction readFunction, void* obj)
{
...
//
bool Udp::open(UINT32 ipAddress, UINT16 port,  bool enableVerboseDebugOutput)
{
	std::string ipAdrStr;
...
//
bool Udp::open(std::string ipAddress, UINT16 port, bool enableVerboseDebugOutput)
{
	INT32 result;
	m_beVerbose = enableVerboseDebugOutput;

//	printInfoMessage("Udp::open: Setting up input buffer with size=" + convertValueToString(requiredInputBufferSize) + " bytes.", m_beVerbose);
//	m_inBuffer.init(requiredInputBufferSize, m_beVerbose);
	
	printInfoMessage("Udp::open: Opening connection.", m_beVerbose);
	
	// Socket erzeugen
	m_connectionSocket = -1;	// Keine Verbindung
	{
		ScopedLock lock(&m_socketMutex);		// Mutex setzen
		m_connectionSocket = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
	}
	if (m_connectionSocket  < 0)
	{
        printError("Udp::open: socket() failed, aborting.");
		return false;
	}

	// Socket ist da. Nun die Verbindung oeffnen.
	printInfoMessage("Udp::open: Connecting. Target address is " + ipAddress + ":" + toString(port) + ".", m_beVerbose);
	...
	if (result < 0)
	{
		...
		std::string text = "Udp::open: Failed to open UDP connection to " + ipAddress + ", aborting.";
		printError(text);
		return false;
	}

	printInfoMessage("Udp::open: Connection established. Now starting read thread.", m_beVerbose);

	// Empfangsthread starten
	m_readThread.run(this);
	
	printInfoMessage("Udp::open: Done, leaving now.", m_beVerbose);
...
//
void Udp::readThreadFunction(bool& endThread, UINT16& waitTimeMs)
{
	INT32 result;
	...
	if (result < 0)
	{
		...
		if (m_readThread.m_threadShouldRun == true)
		{
			...
			printInfoMessage("Udp::readThreadMain: Connection is lost! Read thread terminates now.", m_beVerbose);
...
//
// Read some data from the UDP connection.
//
INT32 Udp::readInputData()
{
	...
	if (isOpen() == false)
	{
		printError("Udp::readInputData: Connection is not open, aborting!");
		return -1;
	}
	...
	{
		// Fehler
		printError("Udp::readInputData: Failed to read data from socket, aborting!");
	}
	else if (recvMsgSize > 0)
	{
		// Erfolg
		printInfoMessage("Udp::readInputData: Read " + toString(recvMsgSize) + " bytes from the connection.", m_beVerbose);
	...
	{
		// Verbindungsabbruch
		printInfoMessage("Udp::readInputData: Read 0 bytes - connection is lost!", true);
...
void Udp::close()
{
	printInfoMessage("Udp::close: Closing Udp connection.", m_beVerbose);
	...
	if (isOpen() == true)
	{
		...
		printInfoMessage("Udp::close: Waiting for the server thread to terminate...", m_beVerbose);
	...
	{
		printInfoMessage("Udp::close: Nothing to do - no open connection? Aborting.", m_beVerbose);
	}

	printInfoMessage("Udp::close: Done - Connection is now closed.", m_beVerbose);
}
 ...
 */
void Udp::stopReadThread()
{
	printInfoMessage("Udp::stopReadThread: Stopping thread.", m_beVerbose);
	
	m_readThread.m_threadShouldRun = false;
	m_readThread.join();

	printInfoMessage("Udp::stopReadThread: Done - Read thread is now closed.", m_beVerbose);
}
 ...
 */
UINT32 Udp::getNumReadableBytes()
{
...
//
UINT32 Udp::read(UINT8* buffer, UINT32 bufferLen)
{
	UINT32 bytesRead = 0;
 ...
 */
std::string Udp::readString(UINT8 delimiter)
{
	UINT8 c = delimiter;
	...
	{
		printInfoMessage("Udp::readString: Returning string: " + outString, true);
	}
